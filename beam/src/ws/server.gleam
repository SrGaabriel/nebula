...
pub fn on_init(
  ...
  #(NebulaState(ws_conn, cableway, option.None, [], dict.new()), option.None)
}
...
pub fn on_message(
  state: NebulaState,
  message: WebsocketMessage(custom),
  ...
  case message {
    mist.Text(text) -> {
      ...
      case state.user_id {
        option.Some(_user_id) -> case decoded {
            Ok(Heartbeat) -> {
                mist.continue(state)
            }
            
            Ok(RealmSubscription(realm_id)) -> {
                let new_subscriptions = subscribe.resubscribe(state, realm_id)
                let all_subscriptions = list.append(state.subscriptions, new_subscriptions)
                let new_state = NebulaState(
                    conn,
                    state.cableway,
                    state.user_id,
                    all_subscriptions,
            ...
            Error(err) -> {
                logging.log(logging.Warning, "Failed to decode WS message: " <> string.inspect(err))
                mist.continue(state)
            }
            _ -> {
                logging.log(logging.Warning, "Received unknown WS message: " <> string.inspect(decoded))
                mist.continue(state)
            }
        }
        option.None -> case decoded {
            Ok(AuthRequest(token)) -> {
                case auth.authenticate_user(state.cableway, token) {
                    option.Some(auth.AuthResponse(_, user_id, realm_perms)) -> {
                      logging.log(logging.Info, "User authenticated: " <> int.to_string(user_id))
                    let new_state = NebulaState(
                        conn,
                        state.cableway,
                        option.Some(user_id),
                    ...
                    option.None -> {
                    logging.log(logging.Warning, "Authentication failed for token: " <> token)
                        mist.stop()
                        }
                    }
                }
                _ -> {
                    logging.log(logging.Warning, "Received non-auth message before authentication")
                    mist.stop()
                }
            }
      }
...
fn decode_message(text: String) -> Result(DecodedMessage, json.DecodeError) {
  let decoder = {
    ...
    case code {
        ...
        5 -> {
            use subscription <- decode.field("data", subscribe.realm_subscription_decoder())
